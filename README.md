...
If on bitcoind v28.0, you will need to make note of the following bitcoind flag set prior to initial block download:  `-blocksxor=0`.
If -blocksxor=1 is set, then Gabriel will need to be made aware of its 8-byte XOR key file.  More on this below.
...
- BLOCK_XOR_KEY_FILE_PATH
  ...
  - If not provided, Gabriel will attempt to auto-discover the XOR key file in the bitcoind data directory.   Subsequently, the following environment variable would need to be set: `BITCOIND_DATA_DIR`.
  - If neither `BLOCK_XOR_KEY_FILE_PATH` nor `BITCOIND_DATA_DIR` are provided, Gabriel will assume that the initial block download of bitcoind was not encrypted using an 8-byte XOR key; ie: `-blocksxor=0` was set during initial block download.
- RUST_LOG
